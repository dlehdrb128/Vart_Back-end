...
type Publicinformation struct {
	...
	Developerleaders Developerleaders
}

// Define the car structure, with 4 properties.  Structure tags are used by encoding/json library
type Basicinfo struct {                            //공시정보 구조체
...
type Tokenprofile struct {                                  //토큰정보 구조체
	Tokenname         string `json:"tokenname"`           //토큰 이름
	Projecttype       string `json:"projecttype"`         //프로젝트 종류
}

type Executives struct {                                    //경영진 구조체
	Name      	string 		`json:"name"`                  //이름
	Education 	string      `json:"education"`             //학력
	Experience string      `json:"experience"`            //경력
}

type Developerleaders struct {                              //개발자리더 구조체
	Name      	string 		`json:"name"`                  //개발자 이름
	Education 	string      `json:"education"`             //개발자 학력	
	Experience string      `json:"experience"`            //개발자 경력
}
...
func (s *Chaincode) initLedgerPubilcinfo(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	for i < len(Publicinformations) {
		fmt.Println("i is ", i)
		PublicinformationAsBytes, _ := json.Marshal(Publicinformations[i]) 
		//network 형태로 전송시키려면 car의 구조체를 bytecode(JSON)로 변환시켜줘야 한다!!
		// '_'를 써서 에러는 처리하지 않음
		APIstub.PutState(Publicinformations[i].No, PublicinformationAsBytes)
...
func (s *Chaincode) addPublicinfo(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var Publicinformations = Publicinformation{
		args[0],
		Basicinfo{
			Companyname: args[1],
